...
static void process_arithmeticm_command(conn *c, token_t *tokens, const size_t ntokens, const bool mult) {
    char temp[INCR_MAX_STORAGE_LEN];
    uint64_t delta;
    char *key;
    size_t nkey;
    ...
    case OK:
        ...
        break;
    case NON_NUMERIC:
        out_string(c, "CLIENT_ERROR cannot multiple or divide non-numeric value");
        break;
    case EOM:
        out_of_memory(c, "SERVER_ERROR out of memory");
        break;
...
void process_command_ascii(conn *c, char *command) {
    ...
    if (first == 'm' && tokens[COMMAND_TOKEN].length == 2) {
        ...
        if (strcmp(tokens[COMMAND_TOKEN].value, "mult") == 0) {
            ...
            process_arithmeticm_command(c, tokens, ntokens, 1);
        } else {
            out_string(c, "ERROR");
        }
    } else if (first == 'g') {
        ...
        if (strcmp(tokens[COMMAND_TOKEN].value, "delete") == 0) {
            ...
            WANT_TOKENS_OR(ntokens, 4, 5);
            process_arithmeticm_command(c, tokens, ntokens, 0);
        } else {
            out_string(c, "ERROR");
        }
    } else if (first == 't') {
